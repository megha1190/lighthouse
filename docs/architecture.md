...
* **Audit** - Tests for a single feature/optimization/metric. Using the Artifacts as input, an audit evaluates a test and  Resolves to a score, which may be pass/fail/numeric.  Meta info may contain markdown for rich text (code, links, etc).
...
* **Audit Category** - Roll-up collection of audits and audit groups into a user-facing section of the report (eg. `Best Practices`). Applies weighting and overall scoring to the section. Examples: PWA, A11y, SEO.
